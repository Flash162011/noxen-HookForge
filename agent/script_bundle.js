...
`;
    ...
    const lock2 = data;
    const methods = lock2.add(lockSize);
...
`, {
  ...
  function makeAddGlobalRefFallbackForAndroid5(api2) {
    ...
    const lock2 = api2.vm.add(offset.globalsLock);
    ...
    return function(vm3, thread, obj) {
      acquire(lock2, thread);
      try {
        return add(table, IRT_FIRST_SEGMENT, obj);
      } finally {
        release(lock2, thread);
      }
    };
  }
...
`;
  ...
  function compileModule(env) {
    ...
    const lock2 = data;
    const models = lock2.add(lockSize);
    ...
    for (var i = 0; i < args.length; i++) {
      try {
        ...
        out.push({
          ...
          value: v === null || v === void 0 ? null : String(v)
        });
      } catch (e) {
        out.push({
          index: i,
    ...
    try {
      ...
      send({
        ...
        "methodReturn": {
          ...
          "value": result === null || result === void 0 ? null : String(result)
        },
  ...
  function processIntercept(className, methodName, intent, pendingIntentFlags, attackSurface, methodArgs) {
    ...
    try {
      ...
      send({
        ...
        "methodArgs": methodArgs || [],
        "pendingIntentFlags": pendingIntentFlags !== void 0 ? pendingIntentFlags : null,
  ...
  function createHook(targetWrapper, className, methodName, overloadArgs, interceptMode) {
    ...
    method.implementation = function() {
      ...
      var shouldPreIntercept = interceptMode === "pre" || interceptMode === "both";
      var shouldReturnIntercept = interceptMode === "return" || interceptMode === "both";
      ...
      try {
        ...
        for (var i = 0; i < perClassCount && results.length < maxResults; i++) {
          ...
          for (var k2 = 0; k2 < keywords.length; k2++) {
            ...
            if (kw2 && lowerMethod.indexOf(kw2) >= 0) {
              methodKeyword = true;
              break;
            }
          }
