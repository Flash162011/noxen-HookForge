...
`;
  ...
  var Java = typeof globalThis.Java !== "undefined" ? globalThis.Java : frida_java_bridge_default;
  try {
    globalThis.Java = Java;
  } catch (_) {
  }
  var config2 = { maxHoldMs: 12e4 };
  ...
  function readField(obj, name) {
    try {
      if (obj && obj[name] !== void 0) {
        var field = obj[name];
        return field && field.value !== void 0 ? field.value : field;
      }
    } catch (_) {
    }
    return null;
  ...
  function serviceFrom(receiver) {
    return readField(receiver, "mService") || readField(receiver, "mAtmService") || readField(receiver, "mActivityTaskManager");
  }
  ...
  function labelMatchesHold(label, hold) {
    ...
    return !!hold.packageName && text.indexOf(String(hold.packageName)) !== -1 || !!hold.processName && text.indexOf(String(hold.processName)) !== -1;
  }
  ...
  function holdForArgs(receiver, args) {
    ...
    for (var i = 0; i < args.length; i++) {
      ...
      var name = readField(arg, "name") || readField(arg, "stringName") || readField(arg, "shortComponentName") || callNoArgs(arg, "getName");
  ...
  function isMissingHookError(error) {
    ...
    return text.indexOf("ClassNotFoundException") !== -1 || text.indexOf("NoClassDefFoundError") !== -1 || text.indexOf("TypeError: cannot read property") !== -1 || text.indexOf("undefined is not an object") !== -1;
  }
  ...
  function installHook(className, methodName, handler) {
    ...
    try {
      ...
      if (!Klass[methodName]) {
        hookStats.push({ className, methodName, installed: 0, status });
        return 0;
      }
  ...
  function installHooks() {
    ...
    count += installHook(
      "com.android.server.wm.AnrController",
    ...
    count += installHook(
      "com.android.server.wm.AnrController",
      "notifyGestureMonitorUnresponsive",
      function(original, returnType, args) {
        var hold = holdForArgs(this, args);
        if (hold) {
  ...
  function capabilitySummary() {
    ...
    hookStats.forEach(function(item) {
      ...
      else if (item.status === "error") errors2 += 1;
      else missing += 1;
    });
    return {
  ...
  rpc.exports = {
    init: function(newConfig) {
      if (newConfig && newConfig.maxHoldMs) {
        config2.maxHoldMs = Math.max(1e3, parseInt(newConfig.maxHoldMs));
      }
    ...
    holdstart: function(hold) {
      if (!hold || hold.pid === void 0 || hold.pid === null) return;
      var pid = String(hold.pid);
      var maxHoldMs = Math.max(1e3, toInt(hold.timeoutMs) || config2.maxHoldMs);
      activeHolds[pid] = {
    ...
    holdend: function(holdId, pid) {
      var pidKey = pid !== void 0 && pid !== null ? String(pid) : null;
