...
def payload_to_history_entry(payload: dict, now: datetime | None = None) -> dict:
    ...
    return {
        ...
        "methodArgs": payload.get("methodArgs") or [],
        "methodReturn": payload.get("methodReturn") or {},
...
def _json_syntax_markup(value) -> str:
    """Return Rich markup with lightweight JSON syntax highlighting."""
    if value is None:
        return ""
    if not isinstance(value, str):
        value = str(value)
    ...
    for match in _JSON_TOKEN_RE.finditer(pretty):
        json_string, key_suffix, number, literal, punct, whitespace, other = match.groups()

        if json_string is not None:
            color = "#79C0FF" if key_suffix is not None else "#A5D6FF"
            parts.append(f"[{color}]{escape(json_string)}[/{color}]")
            if key_suffix:
                parts.append(f"[#C9D1D9]{escape(key_suffix)}[/#C9D1D9]")
        elif number is not None:
            parts.append(f"[#FFA657]{escape(number)}[/#FFA657]")
        elif literal is not None:
            color = "#FF7B72" if literal in ("true", "false") else "#8B949E"
            parts.append(f"[{color}]{escape(literal)}[/{color}]")
        elif punct is not None:
            parts.append(f"[#C9D1D9]{escape(punct)}[/#C9D1D9]")
        elif whitespace is not None:
            parts.append(whitespace)
        elif other is not None:
            parts.append(escape(other))

    return "".join(parts)


def _payload_markup(value) -> str:
    return _json_syntax_markup(value)
...
def render_intercept_block(payload: dict, intercept_counter: int, show_stack: bool, stack_depth: int) -> str:
    ...
    method_args = payload.get("methodArgs") or []

    if method_args:
        out.append("[bold]Arguments:[/bold]")

        for arg in method_args:
            out.append(
                f"  - arg[{_markup(arg.get('index'))}] "
                f"({_markup(arg.get('type'))}): "
                f"{_payload_markup(arg.get('value'))}"
            )

    method_return = payload.get("methodReturn") or {}

    if method_return:
        out.append("[bold]Return Value:[/bold]")
        out.append(
            f"  ({_markup(method_return.get('type'))}): "
            f"{_payload_markup(method_return.get('value'))}"
        )
...
def render_intent_detail(entry: dict, show_stack: bool = False, stack_depth: int = 15) -> str:
    ...
    method_args = entry.get("methodArgs") or []
    ...
    if method_return:
        out.append("  [bold]Return Value[/bold]")
        out.append(
            f"  [dim]({_markup(method_return.get('type'))})[/dim] "
            f"{_payload_markup(method_return.get('value'))}"
        )
