...
from noxen.template_registry import BUILTIN_SCRIPT_TEMPLATES
...
def _compact_json_if_valid(value: str) -> str:
    if value is None:
        return value
    value = str(value)
...
class NoxenApp(App):
    ...
    def __init__(self, cli_args):
        ...
        self._edit_arg_counter = 0
        self._edit_arg_rows: dict = {}
        self._edit_return_type = ""
        self._edit_return_orig = ""
    ...
    def _enter_edit_mode(self):
        ...
        args_container = self.query_one("#ef_args")

        for child in list(args_container.children):
            child.remove()

        self._edit_arg_rows = {}
        self._edit_arg_counter = 0

        for arg in entry.get("methodArgs", []) or []:
            self._add_edit_arg_row(
                int(arg.get("index", 0)),
                str(arg.get("type") or ""),
                str(arg.get("value") or "")
            )

        method_return = entry.get("methodReturn") or {}
        self._edit_return_type = str(method_return.get("type") or "")
        self._edit_return_orig = str(method_return.get("value") or "")

        try:
            return_input = self.query_one("#ef_return_value", Input)
            return_input.value = self._edit_return_orig
            return_input.disabled = not bool(method_return)
        except Exception:
            pass
    ...
    def _add_edit_arg_row(self, index: int, arg_type: str, value: str):
        self._edit_arg_counter += 1
        n = self._edit_arg_counter

        editable = arg_type in (
            "string",
            "java.lang.String",
            "boolean",
            "bool",
        ...
        row = Horizontal(
            Label(f"arg[{index}]", classes="ef_x_key_label"),
            Label(arg_type, classes="ef_x_type_label"),
            Input(id=f"ef_av_{n}", value=value, disabled=not editable),
            classes="ef_x_row",
        )

        self.query_one("#ef_args").mount(row)
    ...
    def _collect_edit_arg_mods(self) -> list:
        arg_mods = []

        for n, row_info in self._edit_arg_rows.items():
            try:
                new_val = self.query_one(f"#ef_av_{n}", Input).value
                orig_val = row_info.get("orig_value", "")

                if new_val != orig_val:
                    arg_type = row_info.get("type") or "java.lang.String"

                    if arg_type == "string":
                        arg_type = "java.lang.String"
    ...
    def _collect_edit_return_mod(self):
        try:
            new_val = self.query_one("#ef_return_value", Input).value
        except Exception:
            return None

        if new_val != self._edit_return_orig:
            new_val = _compact_json_if_valid(new_val)
            return (new_val, self._edit_return_type or "java.lang.String")
    ...
    def _collect_edit_mods(self) -> list:
        ...
        for n, row_info in self._edit_extra_rows.items():
            ...
            try:
                ...
                if key:
                    val = _compact_json_if_valid(val)
    ...
    def _forward_from_edit_mode(self):
        ...
        arg_mods = self._collect_edit_arg_mods()
        return_mod = self._collect_edit_return_mod()
        self._exit_edit_mode()
        self._apply_mods_and_forward_worker(mods, arg_mods, return_mod)

    @work(thread=True)
    def _apply_mods_and_forward_worker(self, mods: list, arg_mods: list | None = None, return_mod=None):
        try:
            ...
            arg_mods = arg_mods or []

            for index, value, arg_type in arg_mods:
                if not self.frida_session.stage_arg_mod(index, value, arg_type, decision_id):
                   self.write_cmd("[red]No matching method argument blocked to modify[/red]")
                   return

            if return_mod is not None:
                return_value, return_type = return_mod
                if not self.frida_session.stage_return_mod(return_value, return_type, decision_id):
                   self.write_cmd("[red]No matching return value blocked to modify[/red]")
                   return
    ...
    def on_button_pressed(self, event: Button.Pressed):
        if event.button.id == "home_refresh":
            ...
            self._do_disconnect()
            return
        elif event.button.id == "discovery_scan":
            self._start_discovery_scan()
            return
        elif event.button.id == "discovery_clear":
            self._clear_discovery_results()
            return
        elif event.button.id == "discovery_export":
            self._export_discovery_hooks()
            return
        ...
        with TabbedContent(id="main_tabs"):
            with TabPane(" Home ", id="tab_home"):
                with Horizontal(id="home_split"):
                    ...
                    with VerticalScroll(id="home_form"):
                        ...
                        with Horizontal(classes="connect_row"):
                            yield Label("Script template", classes="connect_label")
                            yield Select(
                                [(name, name) for name in BUILTIN_SCRIPT_TEMPLATES.keys()],
                                id="home_script_template",
                                value="None",
                                allow_blank=False,
                            )
            ...
            with TabPane(" Intercept ", id="tab_intercept"):
                ...
                with VerticalScroll(id="edit_form"):
                    yield Label("Arguments", classes="ef_section")
                    yield Vertical(id="ef_args")
                    yield Rule()
                    yield Label("Return Value", classes="ef_section")
                    with Horizontal(classes="ef_row"):
                        yield Label("Value", classes="ef_label")
                        yield Input(id="ef_return_value", placeholder="decrypted return value", disabled=True)
                    yield Rule()
                    with Horizontal(classes="ef_row"):
            ...
            with TabPane(" Discovery ", id="tab_discovery"):
                with VerticalScroll(id="discovery_pane"):
                    yield Label("Automatic Method Discovery", id="discovery_title")
                    yield Rule()
                    with Horizontal(classes="connect_row"):
                        yield Label("Class Prefixes", classes="connect_label")
                        yield Input(
                            id="discovery_prefixes",
                            value="",
                            placeholder="optional: leave empty to discover by keywords",
                            select_on_focus=False,
                        )
                    with Horizontal(classes="connect_row"):
                        yield Label("Keywords", classes="connect_label")
                        yield Input(
                            id="discovery_keywords",
                            value="encrypt,decrypt,crypto,cipher,aes,rsa,hmac,sign,verify,key,keystore,secret,token,otp,pin,password,auth,devicebinding,fingerprint,salt,root,emulator,request,response,http",
                            placeholder="comma-separated method/class keywords",
                            select_on_focus=False,
                        )
                    with Horizontal(classes="connect_row"):
                        yield Label("Limit", classes="connect_label")
                        yield Input(id="discovery_limit", value="150", placeholder="max results", select_on_focus=False)
                        yield Button("Scan", id="discovery_scan")
                        yield Button("Clear", id="discovery_clear")
                        yield Button("Export hooks", id="discovery_export")
                    yield RichLog(id="discovery_output", markup=True, highlight=False, auto_scroll=False)
    ...
    def _try_connect(self):
        ...
        try:
            script_template = str(self.query_one("#home_script_template", Select).value or "None")
        except Exception:
            script_template = "None"

        if script_template != "None":
            script_path = f"template:{script_template}"
    ...
    def _parse_csv_input(self, selector: str) -> list[str]:
        try:
    ...
    def _start_discovery_scan(self):
        if not self.frida_session or not self.frida_session.is_ready():
            try:
                self.query_one("#discovery_output", RichLog).write("[red]Session not ready. Connect to an app first.[/red]")
            except Exception:
                pass
            return

        prefixes = self._parse_csv_input("#discovery_prefixes")
        keywords = self._parse_csv_input("#discovery_keywords")
        try:
            max_results = int(self.query_one("#discovery_limit", Input).value.strip() or "150")
        except Exception:
            max_results = 150
        max_results = max(20, min(max_results, 500))

        try:
            out = self.query_one("#discovery_output", RichLog)
            out.clear()
            out.write("[yellow]Scanning loaded classes with safe limits...[/yellow]")
            out.write(f"[dim]Prefixes:[/dim] {', '.join(prefixes) or '<default>'}")
            out.write(f"[dim]Keywords:[/dim] {', '.join(keywords) or '<default>'}")
        except Exception:
            pass

        self._run_discovery_scan_worker(prefixes, keywords, max_results)

    @work(thread=True)
    def _run_discovery_scan_worker(self, prefixes: list[str], keywords: list[str], max_results: int):
        try:
            result = self.frida_session.discover_methods(
                prefixes=prefixes,
                keywords=keywords,
        ...
        self.call_from_thread(lambda r=result: self._render_discovery_results(r))

    def _render_discovery_results(self, result: dict):
        self._discovery_results = list(result.get("results") or [])
        try:
            out = self.query_one("#discovery_output", RichLog)
            out.clear()
            if result.get("error"):
                out.write(f"[red]Discovery failed:[/red] {result.get('error')}")
                return
            out.write(
                f"[green]Discovery completed[/green]  "
                f"loaded_classes={result.get('loadedClasses', 0)}  "
                f"matched_classes={result.get('matchedClasses', 0)}  "
                f"scanned_classes={result.get('scannedClasses', 0)}  "
                f"skipped_classes={result.get('skippedClasses', 0)}  "
                f"results={len(self._discovery_results)}"
            )
            if result.get("keywordOnlyMode"):
                out.write("[dim]Keyword-only mode: scanning non-framework loaded classes by class/method/signature keywords.[/dim]")
            else:
                out.write("[dim]Prefix mode: scanning classes that match the provided prefixes, then filtering methods by keywords.[/dim]")
            out.write("")
            for idx, item in enumerate(self._discovery_results, start=1):
                args = ", ".join(item.get("args") or [])
                out.write(
                    f"[bold]{idx:03d}[/bold] "
                    f"[{item.get('category','Other')}] "
                    f"score={item.get('score',0)} mode={item.get('interceptMode','pre')}\n"
                    f"    {item.get('clazz')}.{item.get('method')}({args}) -> {item.get('returnType','')}"
                )
        except Exception:
            pass
    ...
    def _clear_discovery_results(self):
        ...
        try:
            self.query_one("#discovery_output", RichLog).clear()
        except Exception:
            pass

    def _export_discovery_hooks(self):
        if not self._discovery_results:
            try:
                self.query_one("#discovery_output", RichLog).write("[yellow]No discovery results to export.[/yellow]")
            except Exception:
                pass
            return
        ...
        for item in self._discovery_results:
            key = (item.get("clazz"), item.get("method"), tuple(item.get("args") or []))
            ...
            hooks.append({
                ...
                "interceptMode": item.get("interceptMode") or "pre",
                "discoveryCategory": item.get("category"),
                "discoveryScore": item.get("score"),
            })

        filename = datetime.now().strftime("discovered_hooks_%Y%m%d_%H%M%S.json")
        try:
            with open(filename, "w", encoding="utf-8") as f:
                json.dump(hooks, f, indent=2, ensure_ascii=False)
            self.query_one("#discovery_output", RichLog).write(f"[green]Exported {len(hooks)} hooks to:[/green] {filename}")
        except Exception as e:
            try:
                self.query_one("#discovery_output", RichLog).write(f"[red]Export failed:[/red] {e}")
            except Exception:
                pass
