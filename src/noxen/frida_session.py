...
class FridaSession:
    ...
    def stage_arg_mod(self, index, value, arg_type="java.lang.String", decision_id=None):
        return bool(
            self._script.exports_sync.stage_arg_mod(
                index,
                value,
                arg_type,
                decision_id
    ...
    def stage_return_mod(self, value, return_type="java.lang.String", decision_id=None):
        return bool(
            self._script.exports_sync.stage_return_mod(
                value,
                return_type,
                decision_id
            )
    ...
    def discover_methods(self, prefixes=None, keywords=None, max_results=300, max_classes=200):
        if not self._script:
            return {"results": [], "error": "Session not ready"}

        options = {
            "prefixes": prefixes or [],
            "keywords": keywords or [],
            "maxResults": int(max_results or 300),
            "maxClasses": int(max_classes or 200),
        }

        try:
            return self._script.exports_sync.discover_methods(options)
        except Exception as e:
