...
from noxen.template_registry import load_builtin_template
...
def load_agent_script(extra_script: str | None = None) -> ScriptLoadResult:
    ...
    if extra_script:
        if extra_script.startswith("template:"):
            template_name = extra_script.replace("template:", "", 1)
            try:
                template_code = load_builtin_template(template_name)
                if template_code:
                    code += "\n\n" + template_code
                    messages.append(log_debug(f"Built-in script template loaded: {template_name}", "loader"))
                else:
                    messages.append(log_warning(f"Built-in script template is empty or disabled: {template_name}", "loader"))
            except Exception as e:
                messages.append(log_warning(f"Built-in script template failed to load: {template_name} ({e})", "loader"))
        else:
            try:
